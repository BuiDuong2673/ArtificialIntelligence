...
def enhancedFeatureExtractorDigit(datum):
    ...
    I added a feature which indicates whether the digit has continuous black area
    or discrete black area. This feature can help classifying more accurately between
    digit which has loop (e.g. 0, 8, 9) and digit which doesn't has loop (e.g. 1, 2, ...)
    ##
    """
    ...
    "*** YOUR CODE HERE ***"
    # Define function to explore as much black pixels in the neighborhoods as possible
    def explore_black_area(features_dict, x, y, reached):
        neighborhood = [(x, y)]
        while len(neighborhood) > 0:
            x, y = neighborhood.pop()
            # Skip the current pixel if it is already reached
            # or it does not belong to the black area
            if reached[y][x] == True or features_dict[(x, y)] == 1:
                continue
            # Set the current pixel as reached
            reached[y][x] = True
            # Add left, right, up, down neighbors to the neighborhood
            if x > 0:
            ...
            if y < DIGIT_DATUM_HEIGHT - 1:
                neighborhood.append((x, y + 1))
    # Build boolean list to store information about whether a pixel is visited
    visited = [[False for _ in range(DIGIT_DATUM_WIDTH)] for _ in range(DIGIT_DATUM_HEIGHT)]
    # Initialize the variable to indicate the number of black areas in the digit's image
    num_black = 0
    # Go through each pixels in the image and count the number of times that
    # the explore_black_area function has to be called to discover all the black pixels
    for x in range(DIGIT_DATUM_WIDTH):
        for y in range(DIGIT_DATUM_HEIGHT):
            if visited[y][x] == False and features[(x, y)] == 0:
                explore_black_area(features, x, y, visited)
                num_black += 1
    # Add feature which indicates whether the black area is discrete or continuous
    if num_black == 1:
        features["continuous black"] = 1
    else:
        features["continuous black"] = 0
    return features
